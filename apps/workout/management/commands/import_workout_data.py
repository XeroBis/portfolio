...
class Command(BaseCommand):
    ...
    def import_type_workouts(self, type_workouts):
        for tw_data in type_workouts:
            TypeWorkout.objects.update_or_create(
                id=tw_data['id'],
                defaults={'name_workout': tw_data['name_workout']}
            )
        self.stdout.write(self.style.SUCCESS(f'  Imported {len(type_workouts)} type workouts'))
    ...
    def import_muscle_groups(self, muscle_groups):
        ...
        self.stdout.write(self.style.SUCCESS(f'  Imported {len(muscle_groups)} muscle groups'))
    ...
    def import_equipment(self, equipment_list):
        ...
        self.stdout.write(self.style.SUCCESS(f'  Imported {len(equipment_list)} equipment'))
            ...
            if w_data.get('type_workout_id'):
                try:
                    ...
                    self.stdout.write(self.style.WARNING(
                        f"  TypeWorkout with id {w_data['type_workout_id']} not found"
                    ))
    ...
    def import_strength_logs(self, strength_logs):
        for sl_data in strength_logs:
            try:
                ...
                self.stdout.write(self.style.WARNING(
                    f"  Skipping strength log {sl_data['id']}: {e}"
                ))
        self.stdout.write(self.style.SUCCESS(f'  Imported {len(strength_logs)} strength logs'))
    ...
    def import_cardio_logs(self, cardio_logs):
        for cl_data in cardio_logs:
            try:
                ...
                self.stdout.write(self.style.WARNING(
                    f"  Skipping cardio log {cl_data['id']}: {e}"
                ))
        self.stdout.write(self.style.SUCCESS(f'  Imported {len(cardio_logs)} cardio logs'))
        ...
        for oe_data in one_exercises:
            try:
                ...
                if oe_data.get('content_type_id'):
                    content_type = ContentType.objects.get(id=oe_data['content_type_id'])
                ...
                self.stdout.write(self.style.WARNING(
                    f"  Skipping one exercise {oe_data['id']}: {e}"
                ))
        self.stdout.write(self.style.SUCCESS(f'  Imported {len(one_exercises)} one exercises'))
    ...
    def fix_sequences(self):
        ...
        sql_commands = [
            ...
            "SELECT setval(pg_get_serial_sequence('\"workout_musclegroup\"','id'), coalesce(max(\"id\"), 1), max(\"id\") IS NOT null) FROM \"workout_musclegroup\";",
            "SELECT setval(pg_get_serial_sequence('\"workout_equipment\"','id'), coalesce(max(\"id\"), 1), max(\"id\") IS NOT null) FROM \"workout_equipment\";",
